...
## Architecture
- `LocalInference.kt`: Wraps `LlmInference` from MediaPipe to run the local `.bin` model.
- `RemoteKnowledgeBase.kt`: Uses Retrofit to query an external API.
- `ChatViewModel.kt`: Orchestrates the decision to use local vs. cloud compute.
